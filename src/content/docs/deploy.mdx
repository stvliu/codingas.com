...
可用端点：`/actuator/health`、`/actuator/health/liveness`、`/actuator/health/readiness`、`/actuator/prometheus`、`/actuator/info`。详见 [可观测性](/admin/observability/)。
...
`deployments/package/` 提供 jlink 定制 JRE + jreleaser 打包，生成免 JRE 依赖的二进制发行包：
...
产物为包含定制 JRE 的压缩包，解压后可直接运行，适合无 JDK 环境或离线部署。
